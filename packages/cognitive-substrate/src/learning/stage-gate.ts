import { UsagePattern, PatternType } from './pattern-detector.js';
...
export function detectStageGate(
  indicators: StageIndicators,
  patterns: UsagePattern[],
  _policies: CognitivePolicy[],
  _decisions: Decision[]
): StageGate {
  const scores = {
    [Stage.EARLY]: 0,
  ...
  const detectedStage = (Object.entries(stageConfidence) as Array<[Stage, number]>).reduce((max, curr) =>
...
export function detectToolingMismatch(
  stageGate: StageGate,
  indicators: StageIndicators
